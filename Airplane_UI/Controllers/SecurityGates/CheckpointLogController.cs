...
public class CheckpointLogController : ControllerBase
{
    ...
    public async Task<ActionResult<GetCheckpointLogDto>> Create([FromBody] CreateCheckpointLogDto dto)
    {
        ...
        if (newLog == null)
            return BadRequest("Failed to create checkpoint log.");
    ...
    public async Task<ActionResult<string>> Delete(int id)
    {
        ...
        if (existing == null)
            return NotFound($"Checkpoint log with ID {id} not found.");

        await _service.DeleteAsync(id);

        return Ok($"Checkpoint log with ID {id} deleted successfully.");
    }
}
