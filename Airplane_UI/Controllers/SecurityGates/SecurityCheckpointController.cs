...
namespace Airplane_UI.Controllers.SecurityGates
{
    ...
    public class SecurityCheckpointController : ControllerBase
    {
        ...
        /// <returns>The checkpoint details or 404 if not found.</returns>
        [HttpGet("{id:int}")]
        public async Task<IActionResult> GetById(int id)
        {
            var checkpoint = await _service.GetByIdAsync(id);
            if (checkpoint == null)
                return NotFound($"Checkpoint with ID {id} not found.");
        ...
        /// </summary>
        /// <param name="id">The checkpoint ID.</param>
        /// <returns>Detailed checkpoint data or 404 if not found.</returns>
        [HttpGet("{id:int}/details")]
        public async Task<IActionResult> GetDetails(int id)
        {
            var details = await _service.GetDetailsAsync(id);
            if (details == null)
                return NotFound($"Details for checkpoint ID {id} not found.");
        ...
        {
            if (createDto == null)
                return BadRequest("Invalid checkpoint data.");

            var created = await _service.CreateAsync(createDto);
            if (created == null)
                return BadRequest("Failed to create checkpoint. Terminal not found or name already exists.");
